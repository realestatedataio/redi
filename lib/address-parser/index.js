class AddressParser
{
    ...
    async CleanAddress(d)
    {
        ...
        let regexOrdinals = / (\d+)(ST|ND|RD|TH)/;
        v = v.replace(regexOrdinals, " $1");
        ...
        let validEndings = [];
        directions.forEach((i) => { validEndings.push(i.replace); });
        streetSuffixes.forEach((i) => { validEndings.push(i.replace); });
        ...
        for (let i = 0; i < exploded.length; i++)
        {
            ...
            {
                let regex = new RegExp("^" + directions[j].pattern + "$");
                exploded[i] = exploded[i].replace(regex, directions[j].replace);
            }

            for (let j = 0; j < streetSuffixes.length; j++)
            {
                let regex = new RegExp("^" + streetSuffixes[j].pattern + "$");
                exploded[i] = exploded[i].replace(regex, streetSuffixes[j].replace);
            }

            for (let j = 0; j < ordinals.length; j++)
            {
                let regex = new RegExp("^" + ordinals[j].pattern + "$");
                exploded[i] = exploded[i].replace(regex, ordinals[j].replace);
            }

            for (let j = 0; j < misc.length; j++)
            {
                let regex = new RegExp("^" + misc[j].pattern + "$");
                exploded[i] = exploded[i].replace(regex, misc[j].replace);
            }
        }

        const specificAddressForms = 
        [
            {"pattern": "^([0-9-]+) (AVE) ([A-Z]) "}
        ];
        ...
        for (let i = 0; i < specificAddressForms.length; i++)
        {
            let regex = new RegExp(specificAddressForms[i].pattern);
        ...
        if (result.unit)
        {
            ...
            {
                let regex = new RegExp("^" + units[i].pattern + "$");
                unit = unit.replace(regex, units[i].replace);
                unit = unit.trim();
            }
        ...
        v = exploded.join(" ").trim();
        result.address = d;
        result.cleanAddress = v;
    ...
    async CleanUnit(d, propertyType)
    {
        ...
        [
            ...
            {"pattern": "(UNIT|UNT|UT)(-?)(.+)", "replace": "$3"},
            {"pattern": "(RESIDENCE|RESIDENC|RESIDNCE|RESIDNC|RESID|RES)(-?)(.+)", "replace": "$3"},
            {"pattern": "(TOWNHOUSE|TOWNHOME|TWNHSE|TWNHS|TWHSE|TWNH|TWHM|TWHN|TWN|TWH|TWS|TH)(-?)(.*)", "replace": "TH$3"},
            {"pattern": "(NONE|NOAPT|NO|NA|MISSING)", "replace": propertyType},
            ...
            {"pattern": "(NORTH|NRTH)", "replace": "N"},
            {"pattern": "(SOUTH|SOTH)", "replace": "S"},
            {"pattern": "(EAST|EST)", "replace": "E"},
            {"pattern": "(WEST|WST)", "replace": "W"},
            {"pattern": "(.+)", "replace": "$1"}
        ];

        for (let i = 0; i < basicPatterns.length; i++)
        {
            let regex = new RegExp("^" + basicPatterns[i].pattern + "$");
            v = v.replace(regex, basicPatterns[i].replace);
        }
